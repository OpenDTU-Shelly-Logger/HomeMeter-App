...
async function loadHistoryData(
  baseUrl: string,
): Promise<{ items: DailySolarData[] }> {
  try {
    const response = await fetch(`${baseUrl}/api/data/history`);
...
function getAllHistoryItems(data: string): { items: DailySolarData[] } {
...
export { getAllHistoryItems, loadHistoryData };
